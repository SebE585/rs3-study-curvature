...
def main():
    ...
    ap.add_argument(
        "--docs-rel",
        default=None,
        help="chemin relatif vers les plots pour intégration docs/",
    )
    args = ap.parse_args()

    cfg = yaml.safe_load(open(args.config))
    metrics = cfg.get("metrics", [])
    {m["name"]: m.get("label", m["name"]) for m in metrics}
    ...
    if latest_stats and os.path.exists(latest_stats):
        ...
        print(
            "[WARN] Aucun CSV by_class trouvé — le rapport sera généré en mode *plots-only*."
        )
    ...
    if not plots_parent:
        print(
            "[WARN] Aucun dossier plots 'by_class_' trouvé, le rapport n’inclura pas d’images."
        )
    if (not classes) and plots_parent and os.path.isdir(plots_parent):
        classes = sorted(
            [
                d
                for d in os.listdir(plots_parent)
                if os.path.isdir(os.path.join(plots_parent, d))
            ]
        )
    out_path = args.out
            ...
            for m in (
                ...
                def relp(basename: str) -> str:
                    ...
                    return os.path.relpath(
                        full, start=os.path.dirname(out_path)
                    ).replace("\\", "/")
