...
def _resolve_cols(df: pd.DataFrame):
    cols = {}
    cols["length"] = next(
        (c for c in ["length_m", "len_m", "length"] if c in df.columns), None
    )
    cols["kappa"] = next(
        (
            c
            for c in ["curvature", "curv_mean_1perm", "curv_mean", "kappa"]
            if c in df.columns
        ),
        None,
    )
    cols["radius"] = next(
        (c for c in ["radius_min_m", "radius_m", "radius_p85_m"] if c in df.columns),
        None,
    )
    cols["class"] = next(
        (
            c
            for c in ["class_norm", "highway", "class", "road_class", "fclass"]
            if c in df.columns
        ),
        None,
    )
...
def _flag_turns(
    df: pd.DataFrame, cols: dict, kappa_min: float, r_max: float
) -> pd.Series:
    ...
    cond_k = (
        (k.abs() >= kappa_min) if k is not None else pd.Series(False, index=df.index)
    )
...
def _summarize(
    df: pd.DataFrame, grp: pd.Series, cols: dict, source: str
) -> pd.DataFrame:
    good = grp >= 0
    if good.sum() == 0:
        return pd.DataFrame(
    ...
    L = (
        pd.to_numeric(df2[cols["length"]], errors="coerce")
        if cols["length"]
        else pd.Series(1.0, index=df2.index)
    )
    K = (
        pd.to_numeric(df2[cols["kappa"]], errors="coerce")
        if cols["kappa"]
        else pd.Series(np.nan, index=df2.index)
    )
    R = (
        pd.to_numeric(df2[cols["radius"]], errors="coerce")
        if cols["radius"]
        else pd.Series(np.nan, index=df2.index)
    )
    ...
    for gid, dfc in df2.groupby("curve_gid"):
        ...
        rp85 = (
            np.nanpercentile(R.loc[dfc.index].dropna().values, 15)
            if R.loc[dfc.index].notna().any()
            else np.nan
        )
...
def main():
    ap = argparse.ArgumentParser(
        description="Extraction de virages à partir des segments (OSM/BD)"
    )
    ap.add_argument("--config", required=True)
    ap.add_argument(
        "--kappa-min", type=float, default=1e-4, help="seuil |kappa| minimal (1/m)"
    )
    ap.add_argument(
        "--r-max", type=float, default=150.0, help="seuil rayon maximal (m)"
    )
    ...
    for src_name, df in [("osm", osm), ("bd", bd)]:
        ...
        if cols["kappa"]:
            ...
            print(
                f"[{src_name}] kappa nnz={k.notna().sum()}/{len(k)} min={k.min(skipna=True)} max={k.max(skipna=True)}"
            )
        if cols["radius"]:
            r = pd.to_numeric(df[cols["radius"]], errors="coerce")
            print(
                f"[{src_name}] radius nnz={r.notna().sum()}/{len(r)} min={r.min(skipna=True)} max={r.max(skipna=True)}"
            )
